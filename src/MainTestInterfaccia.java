...
int giocaConAlbero() throws Exception {
    Random r = new Random();
    ...
    Albero alberoVuoto = new Albero(null);
    GiocatoreBotAlbero botAlbero = new GiocatoreBotAlbero("BotAlbero", alberoVuoto, PersonaggiBot);
    ...
    String ultimaDomandaBot  = null;
    String ultimaRispostaBot = null;
    ...
    while (MieiPersonaggi.length > 1 && PersonaggiBot.length > 1) {
        ...
        botAlbero.avanzaERegistra(DomandaRandom, scelta);
        ultimaDomandaBot  = DomandaRandom;
        ultimaRispostaBot = scelta;
        ...
        if (PersonaggiBot.length <= 6 && PersonaggiBot.length > 1) {
            ...
            if (siNo.equals("si")) {
                ...
                for (int i = 0; i < PersonaggiBot.length; i++) NomiPersonaggi.add(PersonaggiBot[i].getNome());
                ...
                if (PersonaggioRandomBot.getNome().equals(IndovinaPersonaggio)) {
                    ...
                    botAlbero.registraFogliaFinale("si", PersonaggioRandomBot);
                    ...
                    while (trovato && i < lista.size()) {
                        if (lista.get(i).getNome().equals(IndovinaPersonaggio)) { lista.remove(i); trovato = false; }
                        i++;
                    }
    ...
    if (PersonaggiBot.length == 1 && ultimaRispostaBot != null) {
        botAlbero.registraFogliaFinale(ultimaRispostaBot, PersonaggioRandomBot);
    }


    Albero alberoDecisionale = botAlbero.getAlberoDecisionale();
    if (alberoDecisionale != null && alberoDecisionale.getRoot() != null) {
        System.out.println("\n=== Albero decisionale del bot ===");
        stampaAlbero(alberoDecisionale.getRoot(), "", true);
        System.out.println("==================================\n");
    }
...
void stampaAlbero(Nodo nodo, String prefisso, boolean ultimo) {
    if (nodo == null) return;
    String ramo = ultimo ? "└── " : "├── ";
    String etichetta = nodo.getPersonaggio() != null
            ? "[" + nodo.getPersonaggio().getNome() + "]"
            : "(" + nodo.getDomanda() + ")";
    System.out.println(prefisso + ramo + etichetta);
    String nuovoPrefisso = prefisso + (ultimo ? "    " : "│   ");

    boolean haSx = nodo.getNodosx() != null;
    boolean haDx = nodo.getNododx() != null;
    if (haSx) { System.out.println(prefisso + (ultimo ? "    " : "│   ") + " NO→"); stampaAlbero(nodo.getNodosx(), nuovoPrefisso, !haDx); }
    if (haDx) { System.out.println(prefisso + (ultimo ? "    " : "│   ") + " SI→"); stampaAlbero(nodo.getNododx(), nuovoPrefisso, true); }
}
...
void main() throws Exception {
    ...
    while (continua) {
        ...
        if (sceltaMenu == MenuPrincipale.SCELTA_GIOCA) {
            ...
            MenuModalita menuModalita = new MenuModalita();
            int modalita = menuModalita.aspettaScelta();

            int risultatoPartita;
            if (modalita == MenuModalita.MODALITA_ALBERO) {
                risultatoPartita = giocaConAlbero();
            } else if (modalita == MenuModalita.MODALITA_CLASSICA) {
                risultatoPartita = gioca();
            } else {
